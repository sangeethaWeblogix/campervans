...
export function buildApiUrl(base: string, filters: FilterState, seed: number, lockCondition?: string): string {
  ...
  if (filters.radius_kms)         params.set("radius_kms",         String(filters.radius_kms));
  ...
  },
  {
    label: (f) => {
      ...
      if (f.from_sleep && f.to_sleep) {
        return String(f.from_sleep) === String(f.to_sleep)
          ? `${f.from_sleep} Berths`
          : `${f.from_sleep} – ${f.to_sleep} Berths`;
      }
...
/** Home > Campervans for Sale > up to 3 active filters — always rendered in
